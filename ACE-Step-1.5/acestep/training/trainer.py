...
def _select_compute_dtype(device_type: str) -> torch.dtype:
    ...
    if device_type == "mps":
        # Use float32 on MPS to avoid NaN gradients with fp16 autocast
...
def _select_fabric_precision(device_type: str) -> str:
    ...
    if device_type in ("cuda", "xpu"):
        return "bf16-mixed"
    if device_type == "mps":
        # Use full fp32 on MPS to avoid NaN gradients; fp16-mixed causes
        # all-NaN grads on Apple Silicon for this model.
        return "32-true"
    return "32-true"
